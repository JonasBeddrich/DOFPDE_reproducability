...
class FDE02RKgraded: 
    ...
    def __init__(self, weights01, exponents01, weights12, exponents12, f, t0, u0, v0, tf, N, gamma, scheme="GL1"): 
    ...
    def solve(self): 
        t_values = self.tf * np.linspace(0,1-1e-12,self.N) ** self.gamma 
        ...
        for i in tqdm(range(1, len(t_values))):
            ...
            self.preprocess(h)
            y_values[i] = self.step(t_values[i-1], y_values[i-1], h)
    ...
    def step(self, tn, yn, h):
        ...
        unc = self.solveNonlinearEquation(tn, yn, h=h)
        vnc = self.computeVNC(un=yn[0], unc=unc, h=h)
        rnc = self.computeRNC(vn=yn[1], vnc=vnc, h=h)
        
        y_next[0] = self.computeSolution(un=yn[0], vnc=vnc, h=h)
        y_next[1] = self.computeDerivative(vn = yn[1], rnc=rnc, h=h)
        y_next[2:2+self.m01] = self.computeModes01(yn=yn, vnc=vnc, h=h)
        y_next[2+self.m01:] = self.computeModes12(yn=yn, rnc=rnc, h=h)

        return y_next

    def computeVNC(self, un, unc, h): 
        return np.matmul(self.Ainv, unc - un * self.vec1) / h

    def computeRNC(self, vn, vnc, h): 
        return np.matmul(self.Ainv, vnc - vn * self.vec1) / h 

    def computeSolution(self, un, vnc, h): 
        return un + h * np.sum(vnc * self.b)
    
    def computeDerivative(self, vn, rnc, h): 
        return vn + h * np.sum(rnc * self.b)

    def computeModes01(self, yn, vnc, h): 
        ...
        return yn[2:2+self.m01] + h * np.matmul(ak_steps, self.b)

    def computeModes12(self, yn, rnc, h): 
        ...
        return yn[2+self.m01:] + h * np.matmul(bj_steps, self.b)

    def solveNonlinearEquation(self, tn, yn, h): 
        tmp = np.zeros(self.RKsteps)
        # from a_k 
        tmp += np.sum(yn[2:self.m01+2]) * self.vec1
        for k in range(self.m01): 
            tmp -= h * self.exponents01[k] * yn[2+k] * np.matmul(self.A, np.matmul(self.Bk[k], self.vec1))
        tmp -= yn[0] * np.matmul(self.D, self.vec1)
        
        # from b_j 
        tmp += np.sum(yn[self.m01+2:]) * self.vec1
        for j in range(self.m12): 
            tmp -= h * self.exponents12[j] * yn[self.m01+2+j] * np.matmul(self.A, np.matmul(self.Cj[j], self.vec1))
        ...
        def nonlinearequation(unc): 
            ...
            for i in range(self.RKsteps): 
                r[i] -= self.f(tn + self.c[i] * h, unc[i])
            return r 
        result = fsolve(nonlinearequation, yn[0] * np.ones(self.RKsteps), xtol=1e-16)      
        return result 
    ...
    def preprocess(self, h): 
        ...
        for k in range(self.m01): 
            self.Bk[k] = np.linalg.inv(np.eye(self.RKsteps) + self.exponents01[k] * h * self.A)

        self.Cj = np.empty(((self.m12, self.RKsteps, self.RKsteps)))
        for j in range(self.m12): 
            self.Cj[j] = np.linalg.inv(np.eye(self.RKsteps) + self.exponents12[j] * h * self.A)
        ...
        self.F = np.matmul(self.E, self.Ainv) / h 
...
def exampleGamma4Sinh(): 
    ...
    def sol(t): 
        return t**3 
    return 0,0, weights01, exponents01, weights12, exponents12, rhs, sol, "exampleGamma4Sinh"
...
def exampleGamma4(): 
    ...
    return u0,v0, weights01, exponents01, weights12, exponents12, rhs, sol, "Gamma4"
...
if __name__ == "__main__":
    ...
    ########################################################################
    u0, v0, weights01, exponents01, weights12, exponents12, rhs, sol, example = exampleSettings("Gamma4")
    ...
    for idx, scheme in enumerate(schemes):
        ...
        for N in Ns: 
            ...
            COFDE = FDE02RKgraded(weights01=weights01, exponents01=exponents01, 
                                  ...
                                  f=rhs, t0=0, u0=np.array([u0]), v0=np.array([v0]), tf=1., N=N, gamma=5, scheme=scheme)
            times, numsol = COFDE.solve()
            error = np.abs(numsol[:,0] - sol(times))
            
            ax_solution[idx].plot(times,numsol[:,0], label='{:0.3e}'.format(N))
            # ax_solution[idx].plot(times,numsol[:,1], label='{:0.3e}'.format(h))
            ax_modes[idx].plot(times,np.sum(numsol[:,2:], axis=1), label='{:0.3e}'.format(N))
            ax_error[idx].plot(times,error, label='{:0.3e}'.format(N))
        ...
        ax_error[idx].set_ylim([1e-10,1e2])

        ax_L1_convergence.set_title(example + " L1 error")
        ax_L1_convergence.plot(Ns, L1error, label=scheme, marker="s", color=cmap(idx/5-1e-12))
        ax_L1_convergence.plot(Ns, (Ns/Ns[0])**orders[scheme] * L1error[0], linestyle="--", color=cmap(idx/5-1e-12))
        ax_L1_convergence.plot(Ns, (Ns/Ns[0])**(orders[scheme]-1) * L1error[0], linestyle=":", color=cmap(idx/5-1e-12))
        ...
        ax_Linf_convergence.plot(Ns, Linferror, label=scheme, marker="s", color=cmap(idx/5-1e-12))
        ax_Linf_convergence.plot(Ns, (Ns/Ns[0])**orders[scheme] * Linferror[0], linestyle="--", color=cmap(idx/5-1e-12))
        ax_Linf_convergence.plot(Ns, (Ns/Ns[0])**(orders[scheme]-1) * Linferror[0], linestyle=":", color=cmap(idx/5-1e-12))
        ...
        ax_LT_convergence.plot(Ns, LTerror, label=scheme, marker="s", color=cmap(idx/5-1e-12))
        ax_LT_convergence.plot(Ns, (Ns/Ns[0])**orders[scheme] * LTerror[0], linestyle="--", color=cmap(idx/5-1e-12))
        ax_LT_convergence.plot(Ns, (Ns/Ns[0])**(orders[scheme]-1) * LTerror[0], linestyle=":", color=cmap(idx/5-1e-12))
